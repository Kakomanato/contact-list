...
const Home = ({ id, Contacts, setContacts, contactId, setContactId }) => {
  ...
  const confirmDeleteContact = () => {
    if (contactToDelete !== null) {
      fetch(`https://playground.4geeks.com/contact/agendas/kako/contacts/${contactToDelete}`, {
        method: 'DELETE'
      })
      .then((response) => {
        if (response.ok) {
  ...
  const confirmDeleteAgenda = () => {
    fetch('https://playground.4geeks.com/contact/agendas/kako', {
      method: 'DELETE',
    })
    .then((response) => {
      if (response.ok) {
  ...
  useEffect(() => {
    ...
    fetch('https://playground.4geeks.com/contact/agendas/kako/contacts')
    .then((response) => response.json())
    .then((data) => {
